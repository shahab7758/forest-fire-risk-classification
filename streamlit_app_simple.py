...
def show_analytics_page():
    ...
    try:
        ...
        # Simple data table instead of charts
        st.subheader("Recent Analyses")
        st.dataframe(df.head(10))
        
        # Risk distribution as text
        st.subheader("Risk Distribution")
        high_risk = len(df[df['prediction'] > 0.7])
        medium_risk = len(df[(df['prediction'] > 0.4) & (df['prediction'] <= 0.7)])
        low_risk = len(df[df['prediction'] <= 0.4])
        
        st.write(f"High Risk: {high_risk} analyses")
        st.write(f"Medium Risk: {medium_risk} analyses")
        st.write(f"Low Risk: {low_risk} analyses")
